...
- ```` ```mermaid ```` fences in any `<Markdown>` render as diagrams: `mermaid-diagram.tsx` lazy-loads the `mermaid` chunk (`securityLevel: 'strict'`, re-rendered on theme change; `look: 'classic'` + `layout: 'tiny'`), shows a fixed-height (500px) fitted preview, and opens `diagram-viewer.tsx` (Dialog with CSS-transform pan/zoom) full screen. Parse failures fall back to the plain code block. Content passed through `TruncatedCode` (tool bodies, conversation files) is deliberately not rendered as diagrams.
